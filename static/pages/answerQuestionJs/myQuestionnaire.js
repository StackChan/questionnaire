...
//                 text += "<span style=\"color:#d9534f;font-size:16px\">暂无调查问卷或问卷已过期</span>";
                 if(projectInfo.list.length == 0){
                	 ...
                	 //text += "<span style=\"font-size:16px\">该项目共有"+projectInfo.list.length+"份调查问卷</span>";
                	 for (var j = 0; j < projectInfo.list.length; j++) {
                         text += "<div style=\"color:#333333;font-size:16px\">" + projectInfo.list[j].questionName + "</div>"
                     }
                 }
                 text += "         </div>";
                 text += "     </div>";
 ...
 function editProject(id, name, content) {
	 var url = '/project/queryIsHaveReleaseQuestionnaire';
	 var data = {
			 "id":id
	 };
	 commonAjaxPost(true,url,data,function (result){
